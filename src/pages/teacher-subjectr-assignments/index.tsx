...
export default function TeacherSubjectAssignmentsPage() {
  ...
  const filteredAssignments = useMemo(() => {
    return assignments.filter((assignment) => {
      if (
        selectedSessionId &&
        assignment.academicSessionId !== selectedSessionId
      ) {
        return false;
      }
  ...
  }, [assignments, selectedSessionId, selectedEmployeeId]);
  ...
  const filteredSections = useMemo(() => {
    ...
    return sections.filter(
      (section) => section.academicSessionId === formAcademicSessionId,
    );
  }, [sections, formAcademicSessionId]);
  ...
  const getSessionName = (sessionId: string) => {
    return (
      academicSessions.find((session) => session.id === sessionId)?.name ||
      "Unknown Session"
    );
  };
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      setError("Please select teacher, subject, section and academic session.");
      return;
    }
                      ...
                      >
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              {getSessionName(assignment.academicSessionId)}
                            </span>
                          </div>
                        </td>
          ...
          <div className="w-full max-w-xl rounded-lg bg-background shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Select the teacher, subject, section and academic session.
                </p>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5 p-6">
              ...
              <div>
                ...
                <select
                  ...
                  required
                  disabled={!formAcademicSessionId}
                ...
                >
                  ...
                  {filteredSections.map((section) => (
                    <option key={section.id} value={section.id}>
                      {getClassName(section.classId)} - Section {section.name}
                    </option>
                  ))}
                </select>
              </div>
